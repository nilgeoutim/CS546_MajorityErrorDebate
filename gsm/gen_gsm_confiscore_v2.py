...
if __name__ == "__main__":
    ...
    for data in questions[:100]:    
        ...
        for round in range(rounds):
            ...
            if all(s < LOW_THRESHOLD for s in scores_this_round):
                ...
                # 下一轮强制 restart prompt：每个 agent 用自己对应的 critic 解释
                for i, agent_context in enumerate(agent_contexts):
                    critic_explanation = critic_explanations_this_round[i]
                    agent_context.append(construct_restart_prompt(question, critic_explanation))
    ...
    json.dump(generated_description, open("gsm_v2_majority_error.json", "w"))

    print("\nSaved gsm_v2_majority_error.json\n")